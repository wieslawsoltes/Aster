...
def main(args):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def privacy():
                clean();w=launch('settings',{'section':'recentitems'});w.get_by_label('Remember recent documents',exact=True).uncheck();page.wait_for_function('!Aster.shellLaunch.state.trackRecent');js("assert(OS.recent.length===0);assert(OS.shellLaunch.state.recent.length===0);assert(OS.shellLaunch.state.pins.length>0);const f=await OS.openPath('/Documents/Launch/note with spaces.txt');await f.ready;await OS.shellLaunch.save();assert(OS.shellLaunch.state.recent.length===0);");page.locator('#taskbar [data-app="notepad"]').click(button='right');j=page.get_by_role('dialog',name='Notepad Jump List');assert j.get_by_role('heading',name='Pinned',exact=True).is_visible();assert j.get_by_role('heading',name='Recent',exact=True).count()==0;page.keyboard.press('Escape')
            ...
            def stale():
                clean();js("await OS.shellLaunch.setTracking(true);await OS.shellLaunch.remember('notepad','/Documents/Launch/hello.html');await OS.fs.remove('/Documents/Launch/hello.html',true);");page.locator('#taskbar [data-app="notepad"]').click(button='right');j=page.get_by_role('dialog',name='Notepad Jump List');entry=j.locator('[data-path="/Documents/Launch/hello.html"]');assert entry.get_by_role('button',name='hello.html',exact=True).is_disabled();entry.get_by_label('Remove hello.html',exact=True).click();page.wait_for_function('!Aster.shellLaunch.state.recent.some(e=>e.path==="/Documents/Launch/hello.html")');js("assert(!await OS.fs.stat('/Documents/Launch/hello.html'));await OS.shellLaunch.setTracking(false);");page.keyboard.press('Escape')
