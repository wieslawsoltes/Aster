...
def main(args):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def text_preview():
                clean();js('await OS.themes.select("macos26-light");');app('files',{'path':'/Documents/Preview'});page.locator('.file-row[data-path="/Documents/Preview/a.html"]').click();page.keyboard.press('Space');panel=page.get_by_role('dialog',name='Quick preview',exact=True);panel.get_by_text('<h1>Unexecuted document</h1>',exact=False).wait_for();js('assert(!window.previewExecuted);assert(!document.querySelector(".quick-preview iframe"));assert(OS.windows.size===1);');page.screenshot(path=str(out/'quick-preview-text.png'));page.keyboard.press('Escape');js('assert(!OS.quickPreview.active&&OS.quickPreview.liveURLs===0);assert(w.el.contains(document.activeElement));')
