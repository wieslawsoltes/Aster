...
def main(args):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def startup():
                launch();page.wait_for_function('w.win32Session.width===154 && w.win32Session.height===182');page.get_by_role('menubar').wait_for(state='visible');page.get_by_role('menuitem',name='Game',exact=True).wait_for(state='visible');assert not args.gpu or page.evaluate('w.win32Session.renderer.mode')=='WebGPU';assert page.get_by_role('menubar').count()==1
