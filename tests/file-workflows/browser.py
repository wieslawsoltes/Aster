...
def main(args):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def drag():
                clean();js("await OS.fs.mkdir('/Documents/Operations/Target');");launch('/Documents/Operations');page.locator('[data-path="/Documents/Operations/b.txt"].file-row').drag_to(page.locator('[data-path="/Documents/Operations/Target"].file-row'));page.wait_for_function('Aster.fileOps.jobs.at(-1)?.status==="done"');js("assert(!await OS.fs.stat('/Documents/Operations/b.txt'));assert(await OS.fs.stat('/Documents/Operations/Target/b.txt'));await OS.fileOps.undo();")
            ...
            def trash():
                clean();launch('/Documents/Operations');page.locator('[data-path="/Documents/Operations/a.txt"].file-row').click();page.keyboard.press('Delete');page.wait_for_function('Aster.fileOps.jobs.at(-1)?.status==="done"');js("assert(!await OS.fs.stat('/Documents/Operations/a.txt'));assert((await OS.fs.list('/.Trash')).some(e=>e.originalPath==='/Documents/Operations/a.txt'));await OS.fileOps.undo();assert(await OS.fs.stat('/Documents/Operations/a.txt'));")
            check('Delete and undo retain the actual Recycle Bin subtree',trash)
            def create():
                clean();launch('/Documents/Operations');page.get_by_role('button',name='New',exact=True).click();page.get_by_role('menuitem',name='Folder',exact=True).click();d=page.get_by_role('dialog',name='New folder',exact=True);d.get_by_role('textbox').fill('Created in Explorer');d.get_by_role('button',name='Save',exact=True).click();page.wait_for_function('Aster.fileOps.jobs.at(-1)?.status==="done"');js("assert(await OS.fs.stat('/Documents/Operations/Created in Explorer'));await OS.fileOps.undo();assert(!await OS.fs.stat('/Documents/Operations/Created in Explorer'));")
