    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            def paste_files():
                ...
                clean();launch('files',{'path':'/Downloads'});js('const data=new DataTransfer();data.items.add(new File(["pasted actual fixture bytes Ω"],"Pasted fixture.txt",{type:"text/plain"}));w.body.querySelector(".explorer-main").dispatchEvent(new ClipboardEvent("paste",{bubbles:true,cancelable:true,clipboardData:data}));');stored('/Downloads/Pasted fixture.txt');js('assert(await OS.fs.text(await OS.fs.read("/Downloads/Pasted fixture.txt"))==="pasted actual fixture bytes Ω");')
