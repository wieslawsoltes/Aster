...
def main(args):
    ...
    with sync_playwright() as pw:
        ...
        def stored(path):
            # wait_for_function polls truthiness; a Promise itself is truthy even
            # when its eventual IndexedDB result is false. Await each actual read.
            js('const until=Date.now()+12000;while(!await OS.fs.stat(arg)){if(Date.now()>until)throw Error("Timed out waiting for committed file: "+arg);await new Promise(r=>setTimeout(r,25));}',path)
        def check(name,fn):
        ...
        try:
            ...
            def native_text():
                ...
                e.evaluate('e=>e.setSelectionRange(7,12)');e.press(primary+'+x');assert e.input_value()=='Before ';e.press(primary+'+z');assert e.input_value()=='Before AFTER';assert not js('return C.model.entries.length;')
            ...
            def manual_workbench():
                e=editor();e.fill('Original must stay');e.click();e.press(end);copy_external('manual workbench Ω');u=utilities();u.get_by_role('button',name='Manual paste…',exact=True).click();d=page.get_by_role('dialog',name='Paste from your clipboard');d.get_by_label('Paste text here').click();d.get_by_label('Paste text here').press(primary+'+v');d.get_by_role('button',name='Use text',exact=True).click();expect(u.get_by_role('textbox',name='Clipboard text')).to_have_value('manual workbench Ω');assert e.input_value()=='Original must stay'
            check('Workbench manual paste updates only its preview, never the remembered editor',manual_workbench)
            def previous_field():
            ...
            def snippets():
                clean();u=utilities();u.get_by_role('tab',name='Snippets').click();u.get_by_role('button',name='Add snippet',exact=True).click();d=page.get_by_role('dialog',name='Add snippet');d.get_by_label('Snippet text').fill('New editable snippet');d.get_by_role('button',name='Save',exact=True).click();u.get_by_role('searchbox').fill('New editable');u.get_by_role('button',name='Edit',exact=True).click();d=page.get_by_role('dialog',name='Edit snippet');d.get_by_label('Snippet text').fill('New editable Ω');d.get_by_role('button',name='Save',exact=True).click();expect(u.locator('.clipboard-snippet')).to_have_text('New editable Ω');assert 'New editable Ω' in u.inner_text()
            ...
            def paste_files():
                ...
                clean();launch('files',{'path':'/Downloads'});page.locator('.explorer-main').click()
                report['fileFixture']=js('const list=w.body.querySelector(".explorer-main");assert(document.activeElement===list,"File fixture must begin with focused list");assert(OS.focused===w.id,"File fixture must own the focused window");const data=new DataTransfer();data.items.add(new File(["pasted actual fixture bytes Ω"],"Pasted fixture.txt",{type:"text/plain"}));assert(data.files.length===1,"File fixture DataTransfer omitted its File");const constructorFileCount=new ClipboardEvent("paste",{clipboardData:data}).clipboardData?.files.length||0;const event=new Event("paste",{bubbles:true,cancelable:true});Object.defineProperty(event,"clipboardData",{value:data});assert(event.clipboardData.files.length===1,"Supplied File missing from synthetic event");list.dispatchEvent(event);return {fixtureTransport:"Event with explicit DataTransfer; not native OS file exposure",constructorFileCount,trusted:event.isTrusted,prevented:event.defaultPrevented,files:event.clipboardData.files.length,types:[...event.clipboardData.types],active:document.activeElement.tagName,focused:OS.focused,owner:w.id};')
            ...
            def address_draft():
                clean();js('await OS.fs.write("/Documents/Address fixture.html","<!doctype html><h1>Address fixture</h1>","text/html");');launch('browser',{'url':'aster://file/Documents/Address%20fixture.html'})
                field=page.get_by_role('textbox',name='Address or search');copy_external('https://example.com/pasted?q=Ω');field.click();field.press(primary+'+a');field.press(primary+'+v');assert field.input_value()=='https://example.com/pasted?q=Ω'
                js('window.addressFrame=w.getWebview().frame;w.getWebview().setZoom(1.1);');assert field.input_value()=='https://example.com/pasted?q=Ω';js('assert(w.getWebview().frame===addressFrame);')
                field.press('Escape');assert field.input_value()=='aster://file/Documents/Address%20fixture.html'
                copy_external('aster://home');field.click();field.press(primary+'+a');field.press(primary+'+v');field.press('Enter');page.get_by_role('dialog',name='Leave the embedded page?').get_by_role('button',name='Cancel',exact=True).click();assert field.input_value()=='aster://home';field.press('Enter');page.get_by_role('dialog',name='Leave the embedded page?').get_by_role('button',name='Continue',exact=True).click();page.wait_for_function('w.browser.current().url==="aster://home"');assert field.input_value()=='aster://home'
            check('Orbit status updates preserve a natively pasted address until submit or Escape',address_draft)
            def terminal():
            ...
            def sdk():
                ...
                js('OS.lock();');f.wait_for_function('!AsterClipboard.connected');page.get_by_role('button',name='Resume your session').click();f.wait_for_function('AsterClipboard.connected');assert f.get_by_label('Guest draft').input_value()=='approved guest text Ω'
                js('await OS.input.configure({clipboardBridge:false});');f.wait_for_function('!AsterClipboard.connected');js('await OS.input.configure({clipboardBridge:true});');f.wait_for_function('AsterClipboard.connected');f.get_by_label('Guest draft').click();assert js('return document.activeElement===w.body.querySelector(".app-frame")&&OS.focused===w.id;');f.get_by_role('button',name='Request clipboard').click();page.get_by_role('dialog',name='Share clipboard text with Clipboard guest?').wait_for();js('await w.close(true);');page.get_by_role('dialog',name='Share clipboard text with Clipboard guest?').wait_for(state='detached')
            check('Opaque guest native paste works; scoped SDK shares only approved text and cancels on close',sdk)
            def off_and_lock():
                e=editor();e.click();js('assert(document.activeElement===w.editor);await C.add("session secret");C.setCurrent({text:"preview secret",html:"",image:null,types:["text/plain"]});OS.lock();assert(!C.current,"Lock retained a clipboard preview");assert(!C.target,"Lock blur reacquired the previous editor target");assert(!C.remember(w.editor),"Locked editor became an insertion target");assert(await C.add("late locked capture")===false,"Locked history accepted new captured text");assert(C.model.entries.every(e=>e.pinned));');page.get_by_role('button',name='Resume your session').click();js('await OS.input.configure({clipboardHistory:false});await OS.featureFlush();assert(!C.model.entries.length);assert(!(await OS.db.get("clipboard-pins")).length);')
