...
def main(args):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def navigation():
                b=page.locator('.window[data-app="browser"]');b.get_by_label('Open pages in').select_option('webview');address=b.get_by_label('Address or search');address.fill(origin+'/fixture/one');address.press('Enter');page.wait_for_function('(url)=>document.querySelector(".orbit-pane:not([hidden]) .browser-iframe")?.getAttribute("src")===url',arg=origin+'/fixture/one');fixture(b.frame_locator('.browser-iframe'));assert b.locator('.browser-iframe').get_attribute('data-browser-policy')=='isolated'
