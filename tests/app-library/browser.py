    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def link_install():
                ...
                window=page.locator('.window').filter(has=page.locator('.app-link-notice'));assert not window.locator('iframe').count();window.get_by_role('heading',name='Open this website in your browser').wait_for();window.get_by_role('button',name='Try embedded webview',exact=True).click();frame=window.locator('iframe');frame.wait_for();assert frame.get_attribute('sandbox')=='allow-scripts allow-forms allow-modals allow-downloads';assert 'allow-same-origin' not in frame.get_attribute('sandbox');assert frame.get_attribute('src')=='https://example.test/docs';js('await linkWindow.close(true);')
                return link
