...
async def main(args):
    ...
    async with async_playwright() as p:
        ...
        await check('Boot with 28 built-in apps and 79 lazy web app entries',lambda:js("assert([...OS.apps.values()].filter(a=>!a.webApp).length===28);assert([...OS.apps.values()].filter(a=>a.webApp).length===79);assert(OS.windows.size===1);assert(document.querySelector('#taskbar button'));return OS.metrics.mode;"))
