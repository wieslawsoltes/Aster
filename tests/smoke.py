...
async def load(page, inject=False, port=8766, url=None):
    if inject:
        ...
        for name in ['core','renderer','windows','apps-files','apps-creative','apps-tools','apps-system','apps-windows','shell']:
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--inject',action='store_true');parser.add_argument('--browser');parser.add_argument('--url');parser.add_argument('--webgpu',action='store_true');args=parser.parse_args()
    server=None
