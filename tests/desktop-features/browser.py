...
def main(args):
    ...
    with sync_playwright() as p:
        ...
        try:
            boot()
            check('Boot registers all ten workflows and preserves 84 web apps',lambda:js("assert([...OS.apps.values()].filter(a=>!a.webApp).length===29);assert(OS.webCatalog.apps.length===84);for(const id of ['files','archives','clipboard','focus','widgets','workspaces','history','storage','accessibility','recorder'])assert(OS.apps.has(id));assert(!document.querySelector('.web-app-frame'));return {apps:OS.apps.size,storage:OS.db.mode};"))
            def explorer():
