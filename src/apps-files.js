    ...
    OS.register('files', { title: 'File Explorer', description: 'Files, folders, and everything in between.', category: 'Essentials', width: 1020, height: 665, minWidth: 430, minHeight: 320,
        mount: async (w, options) => {
            ...
            w.onKey = e => {
                ...
                // A file selection is not a DOM text selection. Request a real Copy
                // transaction on the key gesture so engines that omit the empty-
                // selection copy event still receive the paths and native marker.
                if(OS.input.primary(e)&&!e.shiftKey&&['c','x'].includes(e.key.toLowerCase())&&path!=='/.Trash'){e.preventDefault();copy(e.key.toLowerCase()==='x');return;}
