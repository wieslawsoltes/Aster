...
(() => {
    ...
    OS.previewFiles=async(paths,start=0,owner=null)=>{
        ...
        cover.onkeydown=e=>{
            ...
            if(e.key==='Tab'){const nodes=[...dialog.querySelectorAll('button:not(:disabled),[tabindex="0"],audio,video')];const i=nodes.indexOf(document.activeElement);e.preventDefault();nodes[(i+(e.shiftKey?nodes.length-1:1))%nodes.length]?.focus();}
            e.stopPropagation();
        };
