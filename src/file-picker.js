    ...
    OS.filePicker = {
        ...
        show(config) {
            ...
            function enabled() {
                ...
                confirm.disabled = busy || navigating || navigationFailed || (kind === 'folder' ? !validFolder : kind === 'save' ? !hasName && first?.kind !== 'directory' : !first && !hasName);
            ...
            function select(path, event = {}, focus = true) {
                ...
                // The name follows the remaining selection, not the last clicked row.
                // A folder in Save As is navigation; keep the user's draft name.
                const r = selection.size === 1 ? shown.find(row => selection.has(row.path)) : null;
                if (r?.kind === 'file' && kind !== 'folder') filename.value = M.name(r.path);
                else if (kind === 'open') filename.value = '';
            ...
            async function submit() {
                ...
                try {
                    ...
                    else if (selected.length === 1 && selected[0].kind === 'directory') { busy = false; await navigate(selected[0].path); return; }
                    else if (kind === 'open' && name.trim() && !selected.length) {
