...
(() => {
    ...
    class ScreenRecorder {
        ...
        async start(audio=false){
            ...
            try{
                ...
                this.recorder.onstop=()=>{clearInterval(this.timer);this.stream?.getTracks().forEach(t=>t.stop());this.stream=null;this.blob=this.error||this.disposed?null:new Blob(this.chunks,{type:this.recorder.mimeType||'video/webm'});this.chunks=[];this.state=this.error?'error':'complete';this.resolve?.(this.blob);if(!this.disposed)this.change();};
    ...
    OS.register('recorder',{title:'Screen Recorder',icon:'video',color:'coral',category:'Creation',description:'Record a browser-authorized tab, window or screen; pause, preview and save video.',width:860,height:680,singleton:true,mount(w){
        ...
        w.beforeClose=async()=>!w.dirty&&!['requesting','recording','paused'].includes(record.state)||!!await OS.confirm(w.hostAppId==='snips'?'Leave recording mode?':'Close Screen Recorder?','Active capture stops and unsaved video is discarded.','Close');w.addCleanup(()=>{record.dispose();video.pause();video.removeAttribute('src');video.load();if(url)URL.revokeObjectURL(url);});render();
    }});
})();
