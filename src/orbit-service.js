...
(() => {
    ...
    const service=OS.orbit={
        ...
        siteRoute(url,mode){const origin=new URL(M.webURL(url)).origin;return this.mutate(d=>{d.routes=d.routes.filter(r=>r.origin!==origin);if(mode!=='auto'){if(!['browser','webview'].includes(mode))throw Error('Unknown site opening preference.');if(d.routes.length>=M.LIMITS.routes)throw Error('Remove a site preference first (100 maximum).');d.routes.push({origin,mode});}});},
