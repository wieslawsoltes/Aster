...
(() => {
    ...
    const C=OS.clipboardTools=OS.clipboardText={target:null,model:new M.History(),current:null,
        ...
        async add(text){if(!OS.settings.clipboardHistory||OS.$(".lock-screen"))return false;const entry=this.model.add(text);if(entry)await this.save();return entry;},
        remember(element=deepActive()){
            if(OS.$(".lock-screen")){this.target=null;return null;}
    ...
    function capture(e){if(inserting||e.defaultPrevented||!e.isTrusted||!OS.settings.clipboardHistory||OS.$('.lock-screen'))return;const t=OS.input.target(e);if(privateField(t)||ownUI(t)||!t?.isConnected||privateSelection(t.ownerDocument))return;
